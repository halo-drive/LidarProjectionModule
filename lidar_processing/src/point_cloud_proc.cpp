...
//         // Compute range instead of using point.range
//         float range = std::sqrt(point.x * point.x + point.y * point.y + point.z * point.z);
...
//     // Sort points by computed ring and azimuth for better organization
//     std::sort(organized->begin(), organized->end(), 
//         [](const PointXYZIR& a, const PointXYZIR& b) {
//             // Compute ring for both points
//             auto computeRing = [](const PointXYZIR& point) -> int {
...
//             int ring_a = computeRing(a);  
//             int ring_b = computeRing(b);
...
//         // Remove these - not available in standard pcl::PointXYZI:
//         // point_out.sensor_id = sensor_id;
//         // point_out.timestamp = 0.0f;
        
//         // Don't call calculatePointMetrics since we can't modify the point
//         // calculatePointMetrics(point_out);
